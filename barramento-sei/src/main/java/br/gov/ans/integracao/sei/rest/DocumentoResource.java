...
public class DocumentoResource {
	...
	@Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
	public RetornoInclusaoDocumento incluirDocumento(@PathParam("unidade") String unidade, @QueryParam("template") String template, Documento documento) throws RemoteException, Exception{
	...
	public void validarInclusaoComTemplate(Documento documento) throws BusinessException{
		if(documento.getTipo().equals(Constantes.DOCUMENTO_RECEBIDO)){
			throw new BusinessException("O uso de templates é vedado a documentos gerados.");
		}
	}
	...
	public String transformarConteudoDocumentoInterno(String conteudo, String template) throws RemoteException, Exception{
		try{
			...
			throw new BusinessException("Template "+template+" não encontrado.");
		}
	}
